...
export default function AdminPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setIsAdding(false)
      setFormData({ title: "", author: "", description: "", rating: 0 })
      setCoverFile(null)
      setPdfFile(null)
      fetchNovels()
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-midnight">
        ...
        <div className="flex justify-between items-center mb-10">
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="bg-red-500/20 text-red-400 border border-red-500/30 px-4 py-2 rounded-full hover:bg-red-500/30 transition-all flex items-center gap-2"
            >
              <LogOut size={18} />
              خروج
            </button>
          ...
          <div className="luxury-card p-6 rounded-2xl mb-10">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-lg py-3 px-4"
              ...
              <textarea
                ...
                className="w-full bg-white/5 border border-white/10 rounded-lg py-3 px-4 md:col-span-2"
                rows={4}
              ...
              />
              <div>
                <label className="block text-lunar text-sm mb-2">صورة الغلاف</label>
                <input type="file" accept="image/*" onChange={(e) => setCoverFile(e.target.files?.[0] || null)} />
              </div>
              <div>
                <label className="block text-lunar text-sm mb-2">ملف PDF</label>
              ...
              <div className="md:col-span-2">
                ...
                >
                  {submitting ? "جاري الحفظ..." : "حفظ الرواية"}
                </button>
              </div>
            </form>
          </div>
            ...
            <div key={n.id} className="luxury-card p-4 rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-lunar">{n.author}</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => handleDelete(n.id)} className="p-2 text-red-400 hover:bg-red-500/10 rounded">
                  <Trash2 size={20} />
                </button>
              </div>
            </div>
