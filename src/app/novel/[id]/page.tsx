...
export default function NovelDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!novel) return <div className="p-20 text-center">الرواية غير موجودة</div>
      ...
      <main className="flex-grow container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
          ...
          <div className="md:col-span-2 flex flex-col justify-center text-right">
            ...
            <p className="text-stardust text-lg leading-relaxed mb-10 bg-white/5 p-6 rounded-xl">
              {novel.description}
            </p>
            ...
            <a
              ...
              className="space-button inline-flex items-center gap-2 self-start"
            >
              <Download size={20} />
              تحميل ملف PDF
            </a>
          ...
          <form onSubmit={handleCommentSubmit} className="luxury-card p-6 rounded-2xl mb-12 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-lg py-3 px-4 focus:border-purple-500"
                value={userName}
            ...
            <textarea
              placeholder="شاركنا رأيك في هذه الرواية..."
              className="w-full bg-white/5 border border-white/10 rounded-lg py-3 px-4 focus:border-purple-500"
              rows={4}
              value={newComment}
          ...
          <div className="space-y-6">
            {comments.map((c) => (
              <div key={c.id} className="bg-white/5 p-6 rounded-xl border border-white/10">
                ...
                <span className="text-xs text-lunar mt-4 block">
                  {new Date(c.created_at).toLocaleDateString('ar-EG')}
                </span>
              </div>
            ))}
            {comments.length === 0 && (
              <p className="text-center text-lunar py-10">لا توجد تعليقات بعد. كن أول من يعلق!</p>
            )}
          </div>
        </section>
      </main>
